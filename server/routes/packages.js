...
router.get('/', auth, requirePermission('packages','read'), async (req,res,next)=>{ try { const { limit, offset } = parsePagination(req.query); const { clause, params } = buildSearchFilter(req.query.search,['name']); let base='FROM packages'; if(clause) base+=' WHERE '+clause; const rowsQ=`SELECT * ${base} ORDER BY created_at DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`; const cntQ=`SELECT COUNT(*)::int AS total ${base}`; const [r,c]=await Promise.all([activePool(req).query(rowsQ.replace(/\$SEARCH/g,'$'),[...params,limit,offset]), activePool(req).query(cntQ.replace(/\$SEARCH/g,'$'),params)]); res.json({ data:r.rows, page:{ limit, offset, total:c.rows[0].total }}); } catch(e){ console.error(e); next(new AppError(500,'Error listando paquetes','PACKAGE_LIST_FAIL'));} });
...
router.get('/detailed', auth, requirePermission('packages','read'), async (req,res,next)=>{
	try {
		...
		if (hasItems) {
			...
			`;
			const cntQ = `SELECT COUNT(*)::int AS total FROM packages${where}`;
			...
			const q2 = `SELECT * FROM packages${where} ORDER BY created_at DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`;
			const cntQ2 = `SELECT COUNT(*)::int AS total FROM packages${where}`;
...
router.get('/count', auth, requirePermission('packages','read'), async (req,res,next)=>{
	try { const { rows } = await activePool(req).query('SELECT COUNT(*)::int AS total FROM packages'); res.json({ total: rows[0].total }); }
...
router.post('/', auth, requirePermission('packages','create'), sanitizeBody(['name','description']), validate(packageCreateSchema), audit('create','package', (req,r)=>r.locals?.createdId, (req)=>({ body: req.body })), async (req,res,next)=>{ const { name, description, price } = req.body || {}; try { const { rows } = await activePool(req).query('INSERT INTO packages(name, description, price) VALUES($1,$2,$3) RETURNING *',[name,description||null,price||null]); const created=rows[0]; res.locals.createdId = created.id; res.status(201).json(created); } catch(e){ console.error(e); next(new AppError(500,'Error creando paquete','PACKAGE_CREATE_FAIL'));} });
...
router.put('/:id', auth, requirePermission('packages','update'), sanitizeBody(['name','description']), validate(packageUpdateSchema), audit('update','package', req=>req.params.id, (req)=>({ body: req.body })), async (req,res,next)=>{ const fields=['name','description','price']; const sets=[]; const vals=[]; fields.forEach(f=>{ if(Object.prototype.hasOwnProperty.call(req.body,f)){ sets.push(`${f}=$${sets.length+1}`); vals.push(req.body[f]); }}); if(!sets.length) return next(new AppError(400,'Nada para actualizar','NO_UPDATE_FIELDS')); vals.push(req.params.id); try { const { rows } = await activePool(req).query(`UPDATE packages SET ${sets.join(', ')} WHERE id=$${vals.length} RETURNING *`, vals); if(!rows[0]) return next(new AppError(404,'Paquete no encontrado','PACKAGE_NOT_FOUND')); res.json(rows[0]); } catch(e){ console.error(e); next(new AppError(500,'Error actualizando paquete','PACKAGE_UPDATE_FAIL'));} });

router.delete('/:id', auth, requirePermission('packages','delete'), audit('delete','package', req=>req.params.id), async (req,res,next)=>{ try { const { rowCount } = await activePool(req).query('DELETE FROM packages WHERE id=$1',[req.params.id]); if(!rowCount) return next(new AppError(404,'Paquete no encontrado','PACKAGE_NOT_FOUND')); res.status(204).send(); } catch(e){ console.error(e); next(new AppError(500,'Error eliminando paquete','PACKAGE_DELETE_FAIL'));} });
