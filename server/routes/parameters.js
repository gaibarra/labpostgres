...
// Table expected: system_parameters(id uuid pk default gen_random_uuid(), name text unique not null, value text, created_at timestamptz default now(), updated_at timestamptz default now())
...
router.get('/', requireAuth, requirePermission('settings','view'), async (_req,res,next)=>{
  try {
    ...
    res.json(rows);
  } catch(e){ next(new AppError(500,'Error listando parámetros','PARAMETERS_LIST_FAIL')); }
});
...
router.delete('/:id', requireAuth, requirePermission('settings','update'), async (req,res,next)=>{
  ...
  } catch(e){ next(new AppError(500,'Error eliminando parámetro','PARAMETER_DELETE_FAIL')); }
});
